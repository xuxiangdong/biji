...
​	service iptables {start|stop|reload|restart|condrestart|status|painc|save}

上面命令只是临时关闭，重启服务器还会启动防火墙，如果需要永久关闭，还需执行命令：
...
​		修改文件/etc/ssh/sshd_config中的属性PermitEmptyPasswords为yes
...
```powershell
[root@dfworks97 ~]# ls -l | grep "^d" | wc -l
4
[root@dfworks97 ~]# 
```
...
```powershell
[root@dfworks97 ~]# ls -lR | grep "^-" | wc -l
16518
[root@dfworks97 ~]# 
```
